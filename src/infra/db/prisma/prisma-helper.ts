...
export const PrismaHelper = {
  getProduct: async (
    ...
    SELECT
      ...
      (
        ...
        FROM SG1010 AS SG1COMP (NOLOCK)
          ...
          LEFT JOIN DA1010 (NOLOCK) AS DA1 ON DA1.DA1_CODPRO = SB1COMPONENT.B1_COD AND DA1.DA1_CODTAB = ${priceTable} AND DA1.D_E_L_E_T_ <> '*'
    ...
    FROM SB1010 AS SB1PROD (NOLOCK)
      LEFT JOIN DA1010 (NOLOCK) AS DA1PROD ON DA1PROD.DA1_CODPRO = SB1PROD.B1_COD AND DA1PROD.DA1_CODTAB = ${priceTable} AND DA1PROD.D_E_L_E_T_ <> '*'
    ...
    if (result.length) {
      ...
      if (!!result[0].components?.length) {
        product.components = JSON.parse(result[0].components);

        if (
          !!result[0].totalComponents &&
          result[0].totalComponents - (offset + limit) > 0
        ) {
          product.nextPage = true;
        } else {
          product.nextPage = false;
        }

        if (!!result[0].totalComponents) {
          product.totalComponents = result[0].totalComponents;
        }
      }
